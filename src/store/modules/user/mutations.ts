...
import { semesterDataConverter, stateConverter } from '@/firestore/firestoreconverter'
...
function calculateUserInfo(state: UserState): void {
  ...
  if (current_semester != null) {
    ...
    state.degree_points_to_choose = state.degree_points - state.degree_points_done
    state.degree_points_left = state.degree_points - state.degree_points_done
    state.course_types[mandatory_index].points_left =
      state.course_types[mandatory_index].points_required - english_exemption_points
    ...
    for (const semester of state.semesters.slice().reverse()) {
      ...
      for (const course of semester.courses) {
        ...
        const course_already_done = course.name in courses_done && course_has_number
        ...
        if (
          ...
          if (
            course.name.includes('ספורט') ||
            course.name.includes('גופני') ||
            !(course_already_done && course.number === courses_done[course.name][number_index])
          ) {
          ...
          if (
            ...
            ((course.binary || course_grade > 0) &&
              (course.name.includes('ספורט') || course.name.includes('גופני'))) ||
            (course_already_done &&
              (parseInt(courses_done[course.name][grade_index]) === 0 ||
                courses_done[course.name][binary_index]))
          ) {
            if (
              ...
              state.course_types[course.type].average += course_points * course_grade
          ...
          if (course_info.length > 0) {
            ...
            for (const overlappingKey of first_course_info.overlapping) {
              const full_name = overlappingKey.split(':')
              const course_number = full_name[0]
              const course_name = full_name.slice(1).join().trim()
              courses_done[course_name] = [course_number, course_grade]
            }
            for (const identicalKey of first_course_info.identical) {
              const full_name = identicalKey.split(':')
              const course_number = full_name[0]
              const course_name = full_name.slice(1).join().trim()
              courses_done[course_name] = [course_number, course_grade]
            }
            for (const inclusiveKey of first_course_info.inclusive) {
              const full_name = inclusiveKey.split(':')
              const course_number = full_name[0]
              const course_name = full_name.slice(1).join().trim()
              courses_done[course_name] = [course_number, course_grade]
            }
...
export const mutations: MutationTree<UserState> = {
  ...
  [USER_STORE.MUTATIONS.setActiveSemester](state: UserState, index: number | string) {
    if (index === -1) {
      setTimeout(() => {
  ...
  [USER_STORE.MUTATIONS.setDegreePointsDone](state: UserState, points_done: number) {
    state.degree_points_done = points_done
  },
  [USER_STORE.MUTATIONS.setDegreePointsLeft](state: UserState, points_left: number) {
    state.degree_points_left = points_left
  },
  [USER_STORE.MUTATIONS.setDegreePointsToChoose](state: UserState, points_to_choose: number) {
    state.degree_points_to_choose = points_to_choose
  },
  [USER_STORE.MUTATIONS.addSemester](state: UserState, initial_courses: number) {
    state.summer_semesters = getSummerSemestersNumber(state.semesters)
    state.semesters.push(new Semester(String(state.semesters.length + 1), initial_courses))
  ...
  [USER_STORE.MUTATIONS.addCourseWithDataToLastSemester](state: UserState, course) {
    state.semesters[state.semesters.length - 1].addExistingCourseReturnIndex(course)
    updateUserData(state)
  },
  ...
  [USER_STORE.MUTATIONS.updateCourse](state: UserState, { field, value, index }) {
  ...
  [USER_STORE.MUTATIONS.updateSemesterSummary](state: UserState, { field, value }) {
  ...
  [USER_STORE.MUTATIONS.moveCourse](state: UserState, { index, direction }) {
    ...
    if (
      !(
        (current_semester.courses.length - 1 === index && direction === 'down') ||
  ...
  [USER_STORE.MUTATIONS.changeSemesterType](state: UserState, index: number) {
    console.log(index)
  ...
  [USER_STORE.MUTATIONS.changeCategoryName](state: UserState, { name, index }) {
    if (index < state.course_types.length) {
      state.course_types[index].name = name
    }
  ...
  [USER_STORE.MUTATIONS.moveCourseToSemester](
    state: UserState,
    info: { semester_index: number; course_index: number }
  ) {
    const course_to_move = state.semesters[state.active_semester].courses[info.course_index]
    state.semesters[state.active_semester].courses.splice(info.course_index, 1)
    let index = 0
    for (; index < state.semesters[info.semester_index].courses.length; index++) {
      if (state.semesters[info.semester_index].courses[index].isEmpty()) {
        break
      }
    }
    state.semesters[info.semester_index].courses[index] = course_to_move
  },
  ...
  [USER_STORE.MUTATIONS.checkIfCourseExists](state: UserState, course_number_and_answer) {
    course_number_and_answer['answer'] = courseExistInSemesters(
      state.semesters,
      course_number_and_answer.course_number
    )
  },
  [USER_STORE.MUTATIONS.checkPrerequisites](state: UserState, course_number_and_answer) {
    course_number_and_answer['answer'] = courseExistInSemesters(
      state.semesters,
      course_number_and_answer.course_number,
  ...
  [USER_STORE.MUTATIONS.checkLinear](state: UserState, course_number_and_answer) {
    course_number_and_answer['answer'] = courseExistInSemesters(
      state.semesters,
      course_number_and_answer.course_number,
      state.active_semester
