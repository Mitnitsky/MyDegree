export class Course {
  ...
  isEmpty(): boolean {
    return this.name === '' && (this.number === '' || parseInt(this.number) === 0)
  }
...
export function compareByNumericField(a: Course, b: Course, fieldName: string): number {
  if (fieldName == 'number') {
...
export function is_courses_array_sorted(arr: Course[], fieldName: string): boolean {
